...
# ###############################################################################
PREDICTION_FOLDER ='p:\\ACTIVE\\TigerNET 30966 PBAU\\Kavango\\Assimilation\\Run_2_Predictions_Rundu' #Predictions will be stored here
...
Ass_Out_Folder = 'p:\\ACTIVE\\TigerNET 30966 PBAU\\Kavango\\Assimilation\\Run_2\\Run_2_' # base name of assimilation runs to be read
NBRCH = 12 #number of reaches in the model
REACH_ID = 10 #reach for which in-situ data is available
loop_startdate = date(2012,1,1)
loop_enddate = date(2014,12,2)
ASS_startdate = date2num(date(2005,1,1))
ASS_enddate = date2num(loop_enddate + timedelta(7))
# ###############################################################################
...
period = loop_enddate-loop_startdate + timedelta(1)
enddays = period.days
...
for i in range(0,enddays,1):
    Issue_Date = loop_startdate + datetime.timedelta(i)
    print(Issue_Date)
    if os.path.isfile(OBS_FILE):
        ...
        Ass_Out_Folder_with_date = Ass_Out_Folder + str(Issue_Date)
    if (sobstoday > 0) and os.path.isdir(Ass_Out_Folder_with_date):
        x2 = genfromtxt(Ass_Out_Folder_with_date + os.sep + 'Deterministic_Output.csv', delimiter=',')
        P2 = genfromtxt(Ass_Out_Folder_with_date + os.sep + 'Deterministic_Cov.csv', delimiter=',')
        x3 = genfromtxt(Ass_Out_Folder_with_date + os.sep + 'Assimilation_Output.csv', delimiter=',')
        P3 = genfromtxt(Ass_Out_Folder_with_date + os.sep + 'Assimilation_Cov.csv', delimiter=',')
        q_det = x2[REACH_ID-1,:]
        q_ass = x3[REACH_ID-1,:]
